...
flags.DEFINE_string('video', './data/video/Schräg-80-45-1.mov', 'path to input video or set to 0 for webcam') #'./data/video/test.mp4'
flags.DEFINE_string('output', './data/video/Output.mpeg', 'path to output video')
...
flags.DEFINE_float('score', 0.50, 'score threshold') #0.50
...
def main(_argv):
    ...
    for u in range(1,n_width):
        for v in range (1,n_height):
            sub_frame = frame[(v-1)*size_of_subset:v*size_of_subset,(u-1)*size_of_subset:u*size_of_subset]
            sub_frame_size = sub_frame.shape[:2]
            
            image_data = cv2.resize(sub_frame, (input_size, input_size))
    ...
    print(bbox.shape)
            ...
            for i in bbox:
                cv2.rectangle(frame, (int(bbox[i][0]), int(bbox[i][1])), (int(bbox[i][2]), int(bbox[i][3])), color, 2)
        # draw bbox on screen
...
"""            
        ...
        features = encoder(main_frame, final_bboxes)
        detections = [Detection(bbox, score, class_name, feature) for bbox, score, class_name, feature in zip(final_bboxes, final_scores, names, features)]
        ...
        final_scores = np.array([d.confidence for d in detections])
        classes = np.array([d.class_name for d in detections])
        indices = preprocessing.non_max_suppression(boxs, classes, nms_max_overlap, final_scores)
